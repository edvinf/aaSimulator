...
play_1942se_battle_round <- function(oolAttacker, oolDefender, roundnr, submergeAttack=F, submergeDefend=F, verbose=F, unitlist=aaSimulator::unitlist1942se){
  ...
  if (roundnr == 1){
    ...
    if ("AA" %in% oolDefender){
      stop("Implement 1942 rules for AA gun")
    ...
    #
    stop("Add cruiser to offshore bombbardment")
    ...
    if (cds > 0){
      ...
      cbombhit <- roll(cds, 3)
      remove_casualties(cbombhit, "defender")

      if (verbose & cbombhit > 0){
        write(paste("Hit: ", cbombhit, ". Casualties removed.", sep=""), stdout())
      }
    }

    stop("Handle fireback correctly")
  ...
  stop("Implement assignment of air hits (cannot be assigned tu submarines unless dd present)")

  # subs, submerge after casualties are resolved, if possible (replace with submerged sub, do not add to IPC loss)
  stop("Check if submerge happens before or after firing")
